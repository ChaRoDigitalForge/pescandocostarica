...
export const register = async (req, res, next) => {
  ...
  try {
    ...
    const {
      ...
      last_name,
      phone,
      role = 'cliente', // 'cliente' or 'capitan'
      license_number,
      years_of_experience,
      specializations
    } = req.body;
    ...
    // Validate role
    if (!['cliente', 'capitan'].includes(role)) {
      await client.query('ROLLBACK');
      return res.status(400).json({
        success: false,
        message: 'Invalid role. Must be cliente or capitan'
      });
    }
    ...
    // Convert specializations string to array if provided
