import { getMockTours, getMockTourBySlug } from './mockTours';

const API_URL = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api';
// Habilita el modo mock para desarrollo local sin backend
const USE_MOCK_DATA = process.env.NEXT_PUBLIC_USE_MOCK_DATA === 'true';

export async function getTours(filters = {}) {
  // Si el modo mock está habilitado, usar datos locales
  if (USE_MOCK_DATA) {
    return new Promise((resolve) => {
      setTimeout(() => resolve(getMockTours(filters)), 300);
    });
  }
  try {
...
export async function getFeaturedTours(limit = 6) {
  ...
  if (USE_MOCK_DATA) {
    return new Promise((resolve) => {
      setTimeout(() => resolve(getMockTours({ featured: true, limit })), 300);
    });
  }
...
export async function getTourBySlug(slug) {
  ...
  if (USE_MOCK_DATA) {
    return new Promise((resolve) => {
      setTimeout(() => resolve(getMockTourBySlug(slug)), 300);
    });
  }
